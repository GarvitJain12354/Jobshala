...
import { deleteDiscription, deleteJob, uploadDiscription, uploadJob } from "@/store/Action/action";
import EditResponsibility from "./EditResponsibilty";
...
dispatch(uploadDiscription(info))
setOpen(false)
...
dispatch(deleteDiscription(id))
}
const editeducation = (index)=>{

  setdata(user?.resume?.responsibilty[index]);
  setopenEdit(true)
}
    ...
    <>
      <div className="flex  gap-2 py-6">
        <div className="flex gap-6 items-start ">
          <h1 className="text-md font-normal w-24 mt-6">Jobs</h1>
        </div>
        <div className="flex flex-col gap-1 p-6 px-16 w-[80%] ">
        {user?.resume?.responsibilty.map((dets,index)=>(
...
{/* <h3 className="text-sm">Year of completion:2022</h3> */}
          ...
          >
            <i className="ri-add-fill"></i> Add POSITIONS OF RESPONSIBILITY
          </div>
        </div>
      </div>
