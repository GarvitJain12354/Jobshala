...
import { deleteEducation, deleteSecondary, editEducation, editSecondary, getResume, uploadEducation, uploadSecondary } from "@/store/Action/action";
...
const Secondary = ({setsecondary,open}) => {
...
useEffect(() => {
  user?.resume.secondary?.map((dets)=>(
...
const educationSubmit = (e)=>{
  e.preventDefault();
  if(status === ""){
    return toast.error("Intermediate status  is required");
...
const info = {
secondary:"Senior Secondary or Equivalent (XII)",
  school:school,
  start:start,
  board:board,
  stream:stream,
...
// console.log("hello");
{user?.resume?.secondary?.length !== 0 ? dispatch(editSecondary(secid,info)) : dispatch(uploadSecondary(info))}
setsecondary(false)
}

const deleteeducation = (id)=>{
dispatch(deleteSecondary(id))
}
...
<h3 className="text-sm">{dets?.stream}</h3>
     ...
     <span></span>
      ...
      <span></span>
         ...
         <Button variant="contained" type="submit" className="px-10" style={{ backgroundColor: 'rgb(25, 118, 210)', color: 'white' }}>
          {user?.resume?.secondary?.length !== 0 ? "Update" : "Save" }
