...
import { deleteJob, uploadJob } from "@/store/Action/action";
...
const editeducation = (index)=>{
  ...
  setdata(user?.resume?.jobs[index]);
  setopenEdit(true)
}
  ...
  return (
    <>
      <div className="flex  gap-2 py-6">
        ...
        {user?.resume?.jobs.map((dets,index)=>(
...
<h3>{dets.discription}</h3>
      ...
      >
        <DialogTitle className="w-full text-center text-black text-2xl font-semibold">{"Job Details"}</DialogTitle>
        <DialogContent>
