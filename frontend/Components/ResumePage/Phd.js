...
import { deleteDiploma, deleteEducation, deleteSchool, deleteSecondary, deletephd, editDiploma, editEducation, editSchool, editSecondary, editphd, getResume, uploadDiploma, uploadEducation, uploadScholl, uploadSchool, uploadSecondary, uploadphd } from "@/store/Action/action";
...
// console.log("hello");
{user?.resume?.phd?.length !== 0 ? dispatch(editphd(secid,info)) : dispatch(uploadphd(info))}
setsecondary(false)

}

const deleteeducation = (id)=>{
dispatch(deletephd(id))
}
...
{dets?.percentage ?<h3 className="text-sm">CGPA:{dets?.percentage}/{dets?.performance}</h3> : ""}
      ...
      >
        <DialogTitle className="w-full text-center text-black text-2xl font-semibold">{"PhD details"}</DialogTitle>
        <DialogContent>
    ...
    <MenuItem key={ind} value={year}>
      ...
      <div className="flex items-start flex-col justify-center w-full gap-3 p-2 ">
        ...
        <TextField
              ...
              // label="First Name"
              placeholder="eg. Commerce & Bussiness Studies"
              type="text"
              variant="outlined"
      ...
      <div className="flex items-start flex-col justify-center w-full gap-3 p-2 ">
        ...
        <TextField
         ...
         <Button variant="contained" type="submit" className="px-10" style={{ backgroundColor: 'rgb(25, 118, 210)', color: 'white' }}>
          {user?.resume?.phd?.length !== 0 ? "Update" : "Save" }
